  ...
  return (
    <div className="scale-50 relative w-full max-w-xs flex flex-col items-center" ref={sliderRef}>
      ...
      <div className="relative w-full h-5 flex items-center">
        ...
        <div
          className="absolute left-0 h-1 bg-amber-400 rounded-full z-10"
...
export const HeroSection: React.FC = () => {
  ...
  const [wordGlowHue, setWordGlowHue] = useState(40); // amber/gold hue by default
    ...
    <div className="relative w-full bg-black text-white overflow-hidden">
      <div className="relative z-20 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 h-screen">
        ...
        >
            ...
            <div className="hidden md:flex items-center space-x-6 ml-8">
              {NAV_LINKS.map((link, i) => (
                <Link
                  ...
                  className={
                    i === 0
                      ? "px-4 py-2 bg-gray-800/50 hover:bg-gray-700/50 rounded-full text-sm transition-colors"
                      : "px-4 py-2 text-sm hover:text-gray-300 transition-colors"
                  }
                >
                  {link.label}
                </Link>
          ...
          <div className="flex items-center space-x-4">
            {user ? (
              ...
              <Link to="/login" className="hidden md:block px-4 py-2 text-sm hover:text-gray-300 transition-colors">
                Sign in
              </Link>
            )}
            <button
            ...
            <div className="flex flex-col items-center justify-center h-full space-y-6 text-lg">
              ...
              {NAV_LINKS.map((link, i) => (
                <Link
                  ...
                  className={i === 0 ? "px-6 py-3 bg-gray-800/50 rounded-full" : "px-6 py-3"}
                >
                  {link.label}
                </Link>
              ))}
              {user ? (
                <button className="px-6 py-3" onClick={handleSignOut}>
                  Sign out ({user.name.split(" ")[0]})
                </button>
              ) : (
                <Link to="/login" className="px-6 py-3" onClick={() => setMobileMenuOpen(false)}>
                  Sign in
                </Link>
              )}
              <Link
        ...
        <motion.div variants={containerVariants} initial="hidden" animate="visible" className="w-full z-200 top-[30%] relative">
          <motion.div variants={itemVariants}>
            <FeatureItem name="42,000+" value="headwords" position="left-0 sm:left-10 top-40" />
          </motion.div>
          <motion.div variants={itemVariants}>
            <FeatureItem name="18" value="languages" position="left-1/4 top-24" />
          </motion.div>
          <motion.div variants={itemVariants}>
            <FeatureItem name="Daily" value="new word" position="right-1/4 top-24" />
          </motion.div>
          <motion.div variants={itemVariants}>
            <FeatureItem name="Instant" value="etymology" position="right-0 sm:right-10 top-40" />
          </motion.div>
        </motion.div>
        ...
        >
          <ElasticHueSlider value={wordGlowHue} onChange={setWordGlowHue} label="Adjust Word Glow" />

          <motion.button
            variants={itemVariants}
            whileHover={{ scale: 1.05 }}
            whileTap={{ scale: 0.95 }}
            onClick={() => navigate(user ? "/explore" : "/signup")}
          ...
          >
            <span>{user ? "Start exploring words" : "Start exploring words, free"}</span>
            <svg
              width="16"
              height="16"
              viewBox="0 0 16 16"
          ...
          <motion.h1 variants={itemVariants} className="text-5xl md:text-7xl font-light mb-2 font-serif">
            Lexora
          </motion.h1>
          ...
          <motion.p variants={itemVariants} className="text-gray-400 mb-9 max-w-2xl">
            A dictionary that traces meaning back to its root — definitions, origins, and pronunciation,
            lit up the moment you start typing.
          </motion.p>
      ...
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 1 }} className="absolute inset-0 z-0">
        ...
        <div className="absolute top-0 w-[100%] left-1/2 transform -translate-x-1/2 h-full">
          <Lightning hue={wordGlowHue} xOffset={0} speed={1.6} intensity={0.6} size={2} />
        </div>
        <div className="z-10 absolute top-[55%] left-1/2 transform -translate-x-1/2 w-[600px] h-[600px] backdrop-blur-3xl rounded-full bg-[radial-gradient(circle_at_25%_90%,_#3b2a12_15%,_#000000de_70%,_#000000ed_100%)]"></div>
      </motion.div>
    </div>
  );
};
